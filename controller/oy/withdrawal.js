            ...
            if (!check_saldo.length) {
                ...
                if (saldo - amount > saldo_min) {
                    ...
                    let [results, metadata] = await db.sequelize.query(
                        `INSERT INTO dummy_hold_dana(no_hp, bpr_id, no_rek, nama_rek, token, tcode, ket_trans, reff, amount, tgl_trans, status) VALUES (?,?,?,?,?,?,?,?,?,?,'0')`,
                        {
                        replacements: [
                            no_hp,
                            bpr_id,
                    ...
                    if (!metadata) {
                        ...
                        let [results, metadata] = await db.sequelize.query(
                            `INSERT INTO dummy_transaksi(no_hp, bpr_id, no_rek, nama_rek, tcode, produk_id, ket_trans, reff, amount, tgljam_trans, status_rek) VALUES (?,?,?,?,?,?,?,?,?,?,'0')`,
                            {
                            replacements: [
                                no_hp,
                                bpr_id,
                        ...
                        if (!metadata) {
                            ...
                            res.status(200).send({
                                ...
                                let paramToCombine = [
                                    "POST", 
                                    "/internal-middleware/v2/withdrawal/generatetoken",
                                    timestampMs,
                                ...
                                let token_access = await axios({
                                    method: 'post',
                                    url: `${api_crm}/internal-middleware/v2/withdrawal/generatetoken`,
                                    httpsAgent: agent,
                                    headers: {
                                ...
                                if (token_access.code == "success" && token_access.token) {
                                    let offline_create = await axios({
                                        method: 'post',
                                        url: `${api_offline}/api/offline-create`,
                                        httpsAgent: agent,
                                        headers: {
                                            "Content-Type": "application/json",
                                    ...
                                    if (offline_create.status.code == "102") {
                                        let [results, metadata] = await db.sequelize.query(
                                            `INSERT INTO token(no_hp, bpr_id, no_rek, token, tgl_trans, tgl_expired, status, token_access) VALUES (?,?,?,?,?,?,'0',?)`,
                                            {
                                            replacements: [
                                                no_hp,
                                                bpr_id,
                                        ...
                                        if (!metadata) {
                                            ...
                                            let response = {
                                                ...
                                                reff,
                                                tgl_trans,
                                                tgl_transmis : offline_create.timestamp,
                                                rrn
                                            }
